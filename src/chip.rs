...
impl Chip {
    ...
    pub fn cycle(&mut self) {
        ...
        match (m, x, y, n) {
            ...
            (0x3, _, _, _) => {
                // SE Vx, kk skip next instruction of Vx == kk
            ...
            (0x6, _, _, _) => {
                // LD Vx, kk load kk to Vx
                self.rv[x as usize] = kk as u8;
            }

            (0x7, _, _, _) => {
                // ADD Vx, kk set Vx = Vx + kk
                self.rv[x as usize] = self.rv[x as usize].wrapping_add(kk as u8);
            }
            ...
            (0x8, _, _, 0x6) => {
                // SHR Vx shift right Vx, store LSb in VF
            ...
            (0xD, _, _, _) => {
                // DRW Vx, Vy, n display n-byte sprite starting at location in ri, to coordinates Vx, Vy
                ...
                for h in 0..sprite_hieght {
                    ...
                    for p in 0..8 {
                        // writes each pixel of byte individually
                        ...
                        if pix == 1 && self.display[curr_coor_linear] {
                            // checking collision
                            self.rv[0xF] = 1;
                        }
            ...
            (0xE, _, 0x9, 0xE) => {
                // SKP Vx, skip if key with value of Vx is pressed
            ...
            (0xE, _, 0xA, 0x1) => {
                // SKNP Vx, skip if key with value of Vx is not pressed
                if !self.keyboard[self.rv[x as usize] as usize] {
                    self.pc += 2;
                }
            }
            ...
            (0xF, _, 0x0, 0xA) => {
                // LD Vx, K wait for keypress, load that value into Vx
